...
class Application(object):
    ...
    def dessineResistance(self):
        ...
        for x in range(85,150,24):
            self.ligne.append(self.can.create_rectangle(x, 30, x+15, 70,
                                                        fill='black', width=0))
    ...
    def changeCouleurs(self):
        ...
        if err ==1 or v < 10 or v > 1e11 :
            ...
            v = int(10*v/ordgr+.5)*ordgr/10   # +.5 pour arrondir correctement
            # On recalcule logv et ordgr car v a changé
            logv = int(log10(v))                    
            ordgr = 10**logv
            # On extrait le premier chiffre significatif :
            li[0] = int(v/ordgr)            # partie entière
            decim = v/ordgr - li[0]         # partie décimale
            # On extrait le deuxième chiffre significatif :
